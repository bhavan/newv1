...
function m_photos() {
  ...
  $param['photos'] = db_fetch("select * from `jos_phocagallery` where `catid` = ".$var->get['album_id']." and `published` = 1");
  //fprint($param); _x();
...
function m_upload_photo() {
  ...
  if(isset($var->post['formname']) && $var->post['formname'] == 'upload.event.photo') {
    ...
    if(($image = file_upload(array(
      'name'      => 'image',
      'path'      => $var->joomla_root.'images'.DS.'phocagallery'.DS.'thumbs'.DS,
      ...
      //fprint($sql);
      db_insert($sql);
      @copy($var->joomla_root.'images'.DS.'phocagallery'.DS.'thumbs'.DS.$image, $var->joomla_root.'images'.DS.'phocagallery'.DS.$image);
      @unlink($var->joomla_root.'images'.DS.'phocagallery'.DS.'thumbs'.DS.$image);
...
function m_show_banner($cat) {
  ...
  if ($d['custombannercode'] != "")
	...
	if (strpos('aaa'.$url,'mailto:') || strpos('aaa'.$url,'tel:'))
	 echo '<a href="/adsclick.php?option=com_banners&task=click&bid='.$d['bid'].'"><img src="/images/banners/'.$d['imageurl'].'" alt="'.$d['name'].'" title="'.$d['name'].'" /></a>';
	 else
    echo '<a href="/adsclick.php?option=com_banners&task=click&bid='.$d['bid'].'" target="_blank"><img src="/images/banners/'.$d['imageurl'].'" alt="'.$d['name'].'" title="'.$d['name'].'" /></a>';
}
