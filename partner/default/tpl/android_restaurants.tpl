...
<div id="content">
	...
	<style>
		...
		<ul class="pageitem" style="width:85%;">
			...
			<?php
		$recsub=mysql_query("select * from jos_categories where (parent_id=152 OR id=152) AND section='com_jevlocations2' and published=1 order by `ordering`") or die(mysql_error());
			?>
	...
	<ul class="pageitem">
	  ...
	  while($row=mysql_fetch_array($rec))
	  {
		  $lat2=$row[geolat];
			$lon2=$row[geolon];
	  ?>
      <li class="textbox"><div style="float:left;width:72%;padding-right:5px;">
      <strong><?=$row['title']?></strong><br /><span class="grayplain"><?php echo stripJunk(showBrief(strip_tags($row['description']),30)); ?></span>
        ...
        <div style="float:right;width:26%;vertical-align:top;padding-top:0px;text-align:right;"><?=round(distance($lat1, $lon1, $lat2, $lon2,$dunit),'1')?>&nbsp;<?=$dunit?></div>
