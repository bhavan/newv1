<div id="menu">
	...
	<a id="calPrev" href="javascript:history.go(-1)" onMouseOver="self.status=document.referrer;return true" >
	</a>
</div>
<div id="list" ontouchstart="touchStart(event,'list');" ontouchend="touchEnd(event);" ontouchmove="touchMove(event);" ontouchcancel="touchCancel(event);">
	...
	<div style="width:100%">
		...
		<div style="width:100%">
			...
			?>
			<div style="width:10%;float:left;padding-right:50px;">
				Address:
			</div>
			<div style="width:100%">
	...
	<br />
		...
		Phone:
			...
			<div style="width:90%">
		<a href="tel:<?php echo str_replace(array(' ','(',')','-','.'), '',$row[phone])?>">
		<?=$row[phone]?>
	</a>
	</div>
	</div>
	...
	<div style="width:100%">
	<?=round(distance($lat1, $lon1, $lat2, $lon2,$dunit),'1')?>&nbsp;<?=$dunit?>
	</div>
	</div>
		<br />
			<?php if ($row['url']!=''){ ?>
	...
	<a expr:share_url='data:post.url' href='http://www.facebook.com/sharer.php?u=http://<?php echo $host; ?>/location_details.php?id=<?php echo $did; ?>' name='fb_share' type='box_count'>
	<div class="facebook">
			</div>
			</a>
		</div>
