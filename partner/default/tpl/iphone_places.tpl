		...
		<div id="list">
			...
			<strong>
				Featured
					...
					<strong>
						<?php echo utf8_encode($row_featured['title']); ?>
					</strong>
					<br />
					<span class="grayplain">
						...
						<li>
						<a class="call" href="tel:<?php echo str_replace(array(' ','(',')','-','.'), '', $row_featured[phone]); ?>">
							call
							</a>
						</li>
						<?php } else { ?>
						...
						<?php } else { ?>
						<li>
							<a class="checkin" href="javascript:linkClicked('APP30A:FBCHECKIN:<?php echo $row_featured[geolat]; ?>:<?php echo $row_featured[geolon]; ?>')">
							check in
						</a>
						</li>
							<?php } ?>
							<li>
							<a class="info" href="diningdetails.php?did=<?=$row_featured[loc_id]?>&<?=round($distance_featured,1)?>&lat=<?=$lat1?>&lon=<?=$lon1?>">
							more info
						</a>
					</li>
				<li>
				<a href="javascript:linkClicked('APP30A:SHOWMAP:<?php echo $row_featured[geolon]; ?>:<?php echo $row_featured[geolat]; ?>')">
				</a>
			</li>
			</ul>
				</td>
					<td class="three" style="background:#EFEFEF;">
						<?php echo round($distance_featured,1); ?>&nbsp;<?=$dunit?>
					</td>
					</tr>
						<?php
						}
					...
					<strong>
						...
						<br />
							<span class="grayplain">
							<?php echo stripJunk(showBrief(strip_tags(utf8_encode($row['description'])),30)); ?>
						</span>
						<br />
							<ul>
							<?php if ($_REQUEST['bIPhone']=='0'){?>
						<li>
						<a class="call" href="tel:<?php echo str_replace(array(' ','(',')','-','.'), '', $row[phone]); ?>">
							call
							</a>
							</li>
							<?php } else { ?>
						...
						<?php } else { ?>
						<li>
							<a class="checkin" href="javascript:linkClicked('APP30A:FBCHECKIN:<?php echo $row[geolat]; ?>:<?php echo $row[geolon]; ?>')">
								check in
							</a>
						</li>
						<?php } ?>
					<li>
				<a class="info" href="diningdetails.php?did=<?=$row['loc_id']?>&lat=<?=$lat1?>&lon=<?=$lon1?>">
				more info
				</a>
				</li>
			<li>
			<a href="javascript:linkClicked('APP30A:SHOWMAP:<?php echo $row[geolon]; ?>:<?php echo $row[geolat]; ?>')">
				</a>
					</li>
					</ul>
					</td>
					<td class="three">
						<?php echo round($distance,1); ?>&nbsp;<?=$dunit?>
					</td>
					</tr>
						<?php
						++$n;
					...
					<tr>
						<td class="two">
							<strong>
							<?php echo utf8_encode($data[title]); ?>
						</strong>
						<br/>
							<span class="grayplain">
						...
						<br/>
							<ul>
								<?php if ($_REQUEST['bIPhone']=='0'){?>
							<li>
						<a class="call" href="tel:<?php echo str_replace(array(' ','(',')','-','.'), '', $data[phone]); ?>">
						call
						</a>
							</li>
							<?php } else { ?>
						...
						<a class="call" href="tel:<?php echo str_replace(array(' ','(',')','-','.'), '', $data[phone]); ?>">
							...
							<?php } else { ?>
						<li>
					<a class="checkin" href="javascript:linkClicked('APP30A:FBCHECKIN:<?php echo $data[geolat]; ?>:<?php echo $data[geolon]; ?>')">
					check in
				</a>
				</li>
					<?php } ?>
				<li>
			<a class="info" href="diningdetails.php?did=<?=$data['loc_id']?>&lat=<?=$lat1?>&lon=<?=$lon1?>">
			more info
		</a>
	</li>
<li>
<a  href="javascript:linkClicked('APP30A:SHOWMAP:<?php echo $data['geolon']; ?>:<?php echo $data['geolat']; ?>')">
</a>
</li>
</ul>
</td>
<td class="three">
<?php echo round($dist,1); ?>&nbsp;<?=$dunit?>
</td>
</tr>
<?php } ?>
<?php }
