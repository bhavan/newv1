...
foreach ($this->eventsByRelDay as $relDay => $ebrd) {
	foreach ($ebrd as $row) {
		...
		*/
		...
		//#DD#//
		$new_date = "{$row->_yup}-{$row->_mup}-{$row->_dup}";
		$new_item_link = str_replace("index.php","event_details.php", "{$_SERVER['SCRIPT_NAME']}");
		$new_item_link.="<![CDATA[?event_id={$row->_eventdetail_id}&title={$item_title}&date={$new_date}&rp_id={$row->_rp_id}]]>";
		$item->set('link', $new_item_link);
		//#DD#//
		//$item->set('link', $item_link); //#DD#//
