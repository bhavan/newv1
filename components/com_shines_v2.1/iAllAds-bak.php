...
$db = JFactory::getDBO();
$db->setQuery("SELECT b.*, cat.title as catname FROM #__banner as b LEFT JOIN #__categories as cat ON cat.id=b.catid where b.showBanner=1 AND (b.imptotal=0 OR b.impmade<b.imptotal) AND LOWER(cat.title) IN ('screen','restaurant','main','info','hotspots', 'event') ORDER BY RAND()");
...
if ($ads) foreach ($ads as $ad){
	...
	echo '<Ad id="'.$ad->bid.'" screen="'.$ad->catname.'" image="'.($root."images/banners/".$ad->imageurl).'" type="URL" details="'.htmlspecialchars($url).'" />';
}
echo '</AllAds>';
