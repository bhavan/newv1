...
require_once("../includes/Unzip.php"); //#DD#
...
class PhocaGalleryCpControllerPhocaGalleryu extends PhocaGalleryCpController
{
	...
	//#DD#
		function multipleUploadProcess() {
			...
			$files 			= JRequest::getVar( 'new_file', '', 'files', 'array' );
			...
			{ //#DD#
					...
					if (isset($file['name'])) {
						...
						if (!PhocaGalleryFileUpload::canUpload( $file, $errUploadMsg )) {
							...
							if ($format == 'json') {					
								switch ($errUploadMsg) {
									case 'WARNFILETOOLARGE':
										header('HTTP/1.0 413 Request Entity Too Large');
										jexit(JText::_($errUploadMsg.": {$file['name']}"));
									break;
								...
								JError::raiseNotice(100, JText::_($errUploadMsg.": {$file['name']}"));
								/*
								// REDIRECT
								if ($return) {
									$mainframe->redirect(base64_decode($return).'&folder='.$folder);
						...
						if (JFile::exists($filepath)) {
							if ($format == 'json') {
								header('HTTP/1.0 409 Conflict');
								jexit('Error. File already exists'.": {$file['name']}");
							} else {
								JError::raiseNotice(100, JText::_('Error. File already exists'.": {$file['name']}"));
								/*
								// REDIRECT
								if ($return) {
									$mainframe->redirect(base64_decode($return).'&folder='.$folder);
						...
						if (!JFile::upload($file['tmp_name'], $filepath)) {
							if ($format == 'json') {
								header('HTTP/1.0 406 Not Acceptable');
								jexit('Error. Unable to upload file'.": {$file['name']}");
							} else {
								JError::raiseWarning(100, JText::_('Error. Unable to upload file'.": {$file['name']}"));
								/*
								// REDIRECT
								if ($return) {
									$mainframe->redirect(base64_decode($return).'&folder='.$folder);
							...
							if ($format == 'json') {
								header('HTTP/1.0 400');// With 400 error will be not displayed (?? - ok)
								jexit('Upload complete'.": {$file['name']}");
							} else {
								$mainframe->enqueueMessage(JText::_('Phoca Gallery, Upload complete'.": {$file['name']}"));
								/*
								// REDIRECT
								if ($return) {
									$mainframe->redirect(base64_decode($return).'&folder='.$folder);
					...
					//*****extract if it is zip file*****
					$pos = strpos($file['name'], '.zip');
					if ($pos === false) {
					}else{
						$unzip = new Unzip;
						$unzip_filelist = $unzip->extract($filepath);

						if(count($unzip_filelist)>0){
							foreach($unzip_filelist as $f){
								chmod($f, 777);
							}	
							unlink($filepath);
						}
		...
		{
			echo '<br><input type="file" id="sfile-upload" value="" name="Filedata[]" />';
