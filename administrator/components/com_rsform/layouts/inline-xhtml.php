...
defined( '_JEXEC' ) or die( 'Restricted access' );
	...
	foreach ($info as $r)
	{
		...
		if(in_array($r['ComponentTypeId'],$RSadapter->config['component_ids']))
		{
			...
			$out.= "\t\t".'<div class="formCaption">{'.$r['PropertyValue'].':caption}'.((isset($componentProperties['REQUIRED']) && $componentProperties['REQUIRED']=='YES') ? '<strong class="formRequired">'.(isset($required) ? $required : '(*)').'</strong>' : '' ).'</div>'."\n";
