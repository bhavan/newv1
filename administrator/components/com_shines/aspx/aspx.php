...
<Welcome text="<?php echo $greeting;?>" email="feedback@destinshines.com" />
	 ...
	 */
		...
		$map = file_get_contents(JPATH_SITE."/components/com_shines/ipodhtml/$filename.html");
	...
	$redirect = "/index.php?option=com_jevents&task=day.listevents&tmpl=component";
	...
	*/
	 ...
	 */
		...
		$redirect = "/index.php?option=com_jevents&task=icalrepeat.detail&tmpl=component";
...
// NO iRestaurant
	 ...
	 * 
	 */		
		$redirect = "/index.php?option=com_jevlocations&task=locations.listlocations&tmpl=component";
	 ...
	 */
		$id=JRequest::getInt("id",0);
		$redirect = "/index.php?option=com_jevlocations&task=locations.detail&tmpl=component&loc_id=$id";
		$redirect .= "&iphoneapp=1";
		$la = JRequest::getFloat("la",0);
		$redirect .= "&lat=".$la;
		$lo = JRequest::getFloat("lo",0);
...
// no iCommunity
	 ...
	 */
		...
		if ($ads) foreach ($ads as $ad){
			$url = JURI::root()."index.php?option=com_banners&task=click&bid=".$ad->bid;
...
<RandomAd>
	...
	default:
		echo "not done yet : " .$filename. " ".$script;
		break;
}
