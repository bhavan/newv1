...
class JElementJevcflist extends JElementList
{
	...
	function fetchElement($name, $value, &$node, $control_name, $allowmultiple = false)
	{
		...
		if ($allowmultiple && $node->attributes('multifilter',0)==1){
			if ($value != ""){
				$value = explode(",",$value);
			...
			$size =  ( $node->attributes('filtersize') ? ' size="'.$node->attributes('filtersize').'"' : '' );
	...
	{
		$script =  "JevrRequiredFields.fields.push({'name':'".$control_name.$name."', 'default' :'".$node->attributes('default') ."' ,'reqmsg':'".trim(JText::_($node->attributes('requiredmessage'),true))."'}); ";
		return $script ;
	}
/*
	function fetchCategoryRestrictionScript($name, &$node, $control_name, $cats)
	{
		$script = "JevrCategoryFields.fields.push({'name':'".$name."', 'default' :'".$node->attributes('default') ."' ,'catids':".  json_encode($cats)."}); ";
		return $script;
	}
 */
	...
	public function constructFilter($node){
		...
		if ($this->node->attributes('multifilter',0)==1){
	...
	public function createFilterHTML(){
		...
		if ($this->node->attributes('multifilter',0)==1){
			...
			$script = "JeventsFilters.filters.push({id:'".$this->filterType."_fv',value:".$this->filterNullValue ."});";
		}
