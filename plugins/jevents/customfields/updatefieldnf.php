...
function ProcessRequest(&$requestObject, $returnData){
	...
	// does the new state field exist?
	$newstate = intval($requestObject->newstate);

	$date = JFactory::getDate();
	$value .= "#&^".$user->id."#&^".$date->toFormat("%Y-%m-%d %H:%M")."#&^".$newstate;
	if ($requestObject->append && $field){
		$value = $value."##@@^^".$field->value;
	}

	// I need information about the template here to get the state 'text'
	$plugin = JPluginHelper::getPlugin('jevents', 'jevcustomfields' );
	$pluginparams = new JParameter($plugin->params);
	$template = $pluginparams->get("template","");
	if ($template!=""){
		$xmlfile = dirname(__FILE__)."/templates/".$template;
		$nodedata = new JParameter(array(),$xmlfile);
	}
	...
	foreach ($lines as $line){
		...
		if (count($parts)==4) {
			$cuser = JFactory::getUser($parts[1]);

			// I need information about the template here!
			if ($xmlfile){
				foreach ($nodedata->_xml["_default"]->children() as $node)  {
					if ($node->attributes('name')!=$requestObject->field) continue;
			...
			$retvalues[] = "<div class='nfcomment'><span class='nfstate'>".$parts[3]. "</span>".$parts[0]. " (".$cuser->username." @ ".$parts[2].") </div>";
		}
		else  $retvalues[] = "<div class='nfcomment'>".$line."</div>";
	}
	$returnData->newvalue = nl2br(implode("",$retvalues));
	...
	$db = JFactory::getDBO();
	$sql = "SELECT * FROM #__jev_customfields where name=".$db->Quote($requestObject->field."_state")." and evdet_id=".$event->_detail_id;
	$db->setQuery($sql);
	$field = $db->loadObject();
	if ($field){
		$sql = "UPDATE #__jev_customfields SET value=".$newstate. " WHERE name=".$db->Quote($requestObject->field."_state")." and evdet_id=".$event->_detail_id;
		$db->setQuery($sql);
		$success = $db->query();
	}
	else {
		$sql = "INSERT INTO  #__jev_customfields (value, evdet_id, name ) VALUES(".$newstate.", ".intval($event->_detail_id).", ".$db->Quote($requestObject->field."_state").")";
		$db->setQuery($sql);
		$success = $db->query();
	}
