...
Request : Fetching category id from URL 
Developer:Rinkal 
...
$res=mysql_query("select title from jos_categories where id=".$category_id."");
	while($loc_title=mysql_fetch_array($res))
	{
	$loc_cat_name=$loc_title['title'];
	$loc_cat_name=explode(" -", $loc_cat_name);
...
<html>
<head>
<title><?php echo $var->site_name.' | '.$loc_cat_name[0]; ?></title>
...
<div id="wrapper">
    ...
    <h2><?php echo $var->site_name; ?> <?php echo $loc_cat_name[0]; ?></h2>
    <?php
		/* Code added for generic_locations.tpl */
		require($var->tpl_path."generic_locations.tpl");
	?>
	...
	*/
	...
	$rec=mysql_query("select * from jos_categories where (parent_id=".$category_id." OR id=".$category_id.") AND section='com_jevlocations2' and published=1 order by `ordering`") or die(mysql_error());
